...
class GFPAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        if tgt_agg != 'actor':
            ...
            noises = jax.random.normal(sample_rng, (batch_size, action_dim))
            vabc_acts = self.compute_flow_actions(vabc_obs, noises=noises)
        else:
            vabc_acts = None
        if tgt_agg != 'vabc':
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        target_vabc_acts = self.compute_flow_actions(batch['observations'], noises=noises)
        actor_acts = self.network.select('actor_onestep_flow')(batch['observations'], noises, params=grad_params)
        ...
        # Guidance * error
        pred = self.network.select('va_bc_flow')(batch['observations'], x_t, t, params=grad_params)
    ...
    def compute_flow_actions(
        ...
        if self.config['encoder'] is not None:
            observations = self.network.select('va_bc_flow_encoder')(observations)
        actions = noises
        # Euler method.
        for i in range(self.config['flow_steps']):
            t = jnp.full((*observations.shape[:-1], 1), i / self.config['flow_steps'])
            vels = self.network.select('va_bc_flow')(observations, actions, t, is_encoded=True)
    ...
    def create(
        ...
        if config['encoder'] is not None:
            ...
            encoders['va_bc_flow'] = encoder_module()
        ...
        va_bc_flow_def = ActorVectorField(
            hidden_dims=config['actor_hidden_dims'],
            action_dim=action_dim,
            layer_norm=config['actor_layer_norm'],
            encoder=encoders.get('va_bc_flow'),
        )
        ...
        network_info = dict(
            ...
            va_bc_flow=(va_bc_flow_def, (ex_observations, ex_actions, ex_times)),
            actor_onestep_flow=(actor_onestep_flow_def, (ex_observations, ex_actions)),
        )
        if encoders.get('va_bc_flow') is not None:
            # Add va_bc_flow_encoder to ModuleDict to make it separately callable.
            network_info['va_bc_flow_encoder'] = (encoders.get('va_bc_flow'), (ex_observations,))
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            log_metrics=True,
            target_use_next=False,
            target_agg='actor',
            guidance_agg='actor',
            eta_temperature=0.001,
            guidance_fn='softmax',
